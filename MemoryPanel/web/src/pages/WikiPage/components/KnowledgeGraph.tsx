...
function isStructuralNode(node: GraphNode): boolean {
  ...
  return (
    p.endsWith('/wiki/index.md') ||
    p.endsWith('/wiki/overview.md') ||
    p.endsWith('/wiki/log.md') ||
    p.endsWith('/purpose.md') ||
...
function nc(type: string, palette: AtlasPalette): string {
  return palette.nodeColors[type] || palette.nodeColors.other;
}
...
function layoutIter(n: number): number {
  return n > 2500 ? 28 : n > 1200 ? 40 : n > 600 ? 65 : n > 250 ? 90 : 140;
}
  ...
  useEffect(() => {
    ...
    for (const node of nodes) {
      ...
      graph.addNode(node.id, {
        x: Math.random() * 100,
        y: Math.random() * 100,
        size: ns(node.linkCount, maxLinks, nodes.length),
        color,
        label: node.label,
      });
    }
    ...
    forceAtlas2.assign(graph, {
      iterations: layoutIter(nodes.length),
      settings: {
        ...settings,
        gravity: 1.2,
  ...
  useEffect(() => {
    registerEvents({
      enterNode: (e) => {
        ...
        setHovered({ node: e.node, neighbors: new Set(g.neighbors(e.node)) });
      ...
      clickNode: (e) => {
        const n = nodes.find((n) => n.id === e.node);
    ...
    sigma.setSetting('nodeReducer', (node, data) => {
      ...
      if (hovered) {
        if (node === hovered.node) {
          res.highlighted = true;
          res.zIndex = 2;
          res.size = (data.size || BASE_NODE_SIZE) * 1.3;
    ...
    sigma.setSetting('edgeReducer', (edge, data) => {
      ...
      if (hovered) {
        ...
        if (g.source(edge) !== hovered.node && g.target(edge) !== hovered.node) {
          ...
          res.size = Math.max((data.size || 1) * 1.8, 2);
        }
      }
      return res;
    });
...
function GraphControls() {
  const sigma = useSigma();
  const cls =
    'h-7 w-7 bg-card/90 hover:bg-card border border-border text-muted-foreground shadow-md rounded-md flex items-center justify-center transition-colors text-[12px] backdrop-blur';
  return (
...
export default function KnowledgeGraph({
  ...
  const filteredData = useMemo(
    () => (data ? (hideStructural ? filterStructuralNodes(data) : data) : null),
    [data, hideStructural],
  );
  ...
  const handleSearch = useCallback(
    (q: string) => {
      ...
      setSearchResults(
        filteredData.nodes
          .filter(
            (n) => n.label.toLowerCase().includes(lower) || n.id.toLowerCase().includes(lower),
          )
          .slice(0, 8),
      );
    },
  ...
  if (loading)
    return (
      ...
      >
        <span className="text-xs text-muted-foreground">{t('graph.loading')}</span>
      </div>
    );
  if (!filteredData || filteredData.nodes.length === 0)
    return (
      <div
        ...
        style={{ background: palette.bg }}
      >
        <span className="text-[12px] text-muted-foreground/70">{t('graph.empty')}</span>
      </div>
    );
  ...
  return (
    ...
    >
      {/* Toolbar */}
      <div
        className="flex items-center gap-2 border-b border-border px-3 py-2 z-10"
        style={{ background: palette.toolbarBg, backdropFilter: 'blur(8px)' }}
      >
        <div className="relative flex-1 max-w-[180px]">
          <span className="absolute left-2 top-1/2 -translate-y-1/2 text-muted-foreground/70 text-xs inline-flex items-center">
            <SearchIcon size={12} />
          </span>
          <input
          ...
          {searchQuery && (
            <button
              ...
              className="absolute right-2 top-1/2 -translate-y-1/2 text-muted-foreground/70 hover:text-muted-foreground text-xs inline-flex items-center"
              onClick={() => {
                setSearchQuery('');
                setSearchResults([]);
              }}
        ...
        <div className="flex gap-0.5">
          <button
            className={`rounded-md px-2 py-1 text-xs font-medium transition ${colorMode === 'type' ? 'bg-primary/5 text-primary ring-1 ring-primary/30' : 'text-muted-foreground hover:text-foreground/70 hover:bg-muted'}`}
            onClick={() => setColorMode('type')}
          >
            {t('graph.colorMode.type')}
          </button>
          <button
            className={`rounded-md px-2 py-1 text-xs font-medium transition ${colorMode === 'community' ? 'bg-primary/5 text-primary ring-1 ring-primary/30' : 'text-muted-foreground hover:text-foreground/70 hover:bg-muted'}`}
            onClick={() => setColorMode('community')}
          >
            {t('graph.colorMode.community')}
          </button>
        </div>
        <button
          className={`rounded-md px-2 py-1 text-xs font-medium transition ${hideStructural ? 'bg-success/10 text-success ring-1 ring-success/30' : 'text-muted-foreground hover:text-foreground/70 hover:bg-muted'}`}
          onClick={() => setHideStructural(!hideStructural)}
          title={t('graph.hideStructural.title')}
        >
          {t('graph.hideStructural')}
        </button>
        <span className="text-xs ml-auto font-mono text-muted-foreground">
          {t('graph.stats', { nodes: filteredData.nodes.length, edges: filteredData.edges.length })}
        </span>
      </div>
      ...
      {searchResults.length > 0 && searchQuery && (
        <div className="absolute top-12 left-3 z-20 w-[190px] rounded-lg border shadow-xl p-1 max-h-[200px] overflow-auto bg-card/95 border-border backdrop-blur">
          {searchResults.map((n) => (
            <div
              key={n.id}
              className="flex items-center gap-1.5 rounded-md px-2 py-1 text-xs cursor-pointer hover:bg-muted"
              onClick={() => {
                onNodeClick?.(n);
                setSearchQuery('');
                setSearchResults([]);
            ...
            >
              <div
                ...
                style={{ background: nc(n.type, palette) }}
              />
      ...
      <div className="flex-1 min-h-0 relative" style={gridBg}>
        <SigmaContainer
          ...
          settings={{
            ...
            labelSize: 12,
            labelWeight: '500',
            labelDensity: (data?.nodes?.length || 0) > 600 ? 0.12 : 0.35,
      ...
      {/* Legend */}
      <div
        className="flex flex-wrap items-center gap-3 border-t border-border px-3 py-1.5 z-10"
        style={{ background: palette.toolbarBg, backdropFilter: 'blur(8px)' }}
      >
        {types.map((type) => {
          ...
          return (
            <div key={type} className="flex items-center gap-1.5">
              <div
                ...
                style={{ background: c, boxShadow: isAccent ? 'var(--tea-shadow-xs)' : 'none' }}
              />
