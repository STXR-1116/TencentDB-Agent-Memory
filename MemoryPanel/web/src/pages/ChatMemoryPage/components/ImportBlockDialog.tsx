...
type ParseResult = { ok: true; messages: ImportMessage[] } | { ok: false; error: string };
...
export function ImportBlockDialog({
  ...
  const parsed = useMemo((): ParseResult => {
    ...
    try {
      ...
      return { ok: false, error: t('importBlock.parse.failed') };
    }
    ...
    if (p.length > MAX_MESSAGES)
      return {
        ok: false,
        error: t('importBlock.parse.tooMany', { max: MAX_MESSAGES, count: p.length }),
      };
    const messages: ImportMessage[] = [];
    for (let i = 0; i < p.length; i++) {
      const m = p[i] as any;
      if (!m || typeof m !== 'object')
        return { ok: false, error: t('importBlock.parse.notObject', { index: i + 1 }) };
      const role = m.role;
      if (role !== 'user' && role !== 'assistant')
        return { ok: false, error: t('importBlock.parse.invalidRole', { index: i + 1, role }) };
      const content = m.content;
      if (typeof content !== 'string' || content.length === 0)
        return { ok: false, error: t('importBlock.parse.invalidContent', { index: i + 1 }) };
  ...
  async function submit() {
    ...
    try {
      await onImported({
        ...
        messages: (parsed as { ok: true; messages: ImportMessage[] }).messages,
      });
    } finally {
      ...
      <Modal.Body>
        ...
        <Alert type="info" style={{ marginTop: 12 }}>
          <div className="space-y-1">
            ...
            <ul className="list-disc pl-5 text-[11px] space-y-0.5">
              <li>
                <code className="text-[11px]">role</code> {t('importBlock.format.role')}
                <code className="text-[11px]">"user"</code> /{' '}
                <code className="text-[11px]">"assistant"</code>
              </li>
              <li>
                <code className="text-[11px]">content</code>：{t('importBlock.format.content')}
              </li>
              <li>
                {t('importBlock.format.max')} <strong>{MAX_MESSAGES}</strong>{' '}
                {t('importBlock.format.messages')}
              </li>
            </ul>
          </div>
        </Alert>
            ...
            options={[
              {
                ...
                text: (
                  <>
                    <FilePasteIcon size={12} /> {t('importBlock.mode.paste')}
                  </>
                ),
              },
              {
                ...
                text: (
                  <>
                    <UploadIcon size={12} /> {t('importBlock.mode.file')}
                  </>
                ),
              },
            ]}
          ...
          {importMode === 'paste' ? (
            ...
            <Form.Item label={t('importBlock.file.label')}>
              <Upload accept=".json,.txt,.md" beforeUpload={handleFilePicked}>
                <Button>{t('importBlock.file.select')}</Button>
              </Upload>
              {fileName && (
                <Text theme="text" parent="div" style={{ marginTop: 6 }}>
                  {t('importBlock.file.selected')}
                  <Text parent="code">{fileName}</Text>
                </Text>
              )}
              {sessionPayload && (
                <Form.Item label={t('importBlock.file.preview')} style={{ marginTop: 8 }}>
                  <pre className="w-full max-h-48 overflow-y-auto rounded-lg border bg-muted/50 px-2 py-1.5 text-[10px] font-mono text-foreground/70 whitespace-pre-wrap">
                    {sessionPayload.slice(0, 2000)}
                    {sessionPayload.length > 2000 ? t('importBlock.file.truncated') : ''}
                  </pre>
                </Form.Item>
              )}
            </Form.Item>
        ...
        {sessionPayload.trim() && parsed.ok && (
          <Alert type="success" style={{ marginTop: 12 }}>
            {t('importBlock.parse.success', {
              count: parsed.messages.length,
              user: parsed.messages.filter((m) => m.role === 'user').length,
              assistant: parsed.messages.filter((m) => m.role === 'assistant').length,
            })}
          </Alert>
        )}
        {sessionPayload.trim() && !parsed.ok && (
