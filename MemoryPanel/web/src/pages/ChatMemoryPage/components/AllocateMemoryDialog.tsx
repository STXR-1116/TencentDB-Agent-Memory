...
export function AllocateMemoryDialog({
  ...
  const description =
    memorySource === 'team' ? (
      <>
        {t('allocMemory.desc.team')}{' '}
        <Text theme="strong" parent="span">
          {memoryTitle}
        </Text>{' '}
        {t('allocMemory.desc.team2')}
      </>
    ) : (
      <>
        {t('allocMemory.desc.personal')}{' '}
        <Text theme="strong" parent="span">
          {memoryTitle}
        </Text>{' '}
        {t('allocMemory.desc.personal2')}
      </>
    );
  ...
  return (
    ...
    >
      <Modal.Body>
        <Form>
          <Form.Item label={t('allocMemory.descLabel')}>
          ...
          {agents.length === 0 ? (
            ...
            <Form.Item label={t('allocMemory.agent')} required>
              <Select
                ...
                value={agentId}
                onChange={setAgentId}
                placeholder={t('allocMemory.agent.placeholder')}
      ...
      <Modal.Footer>
        <Button
          ...
          loading={submitting}
        >
          {t('allocMemory.submit')}
        </Button>
        <Button onClick={onClose} disabled={submitting}>
          {t('allocMemory.cancel')}
        </Button>
      </Modal.Footer>
    </Modal>
  );
